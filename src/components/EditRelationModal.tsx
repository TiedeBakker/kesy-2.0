...
export default function EditRelationModal({
  ...
  useEffect(() => {
    if (relationValue) {
      ...
      setValidTo(
        relationValue.validTo ? toLocalDatetimeInput(relationValue.validTo) : ""
      );
      setError(null);
    }
  }, [relationValue]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-md p-6 space-y-4 shadow-2xl">
        ...
        <div className="grid grid-cols-2 gap-3 pt-1">
          <div className="space-y-1">
            <label className="text-xs text-slate-300 font-semibold block">Geldig vanaf</label>
            <input
              type="datetime-local"
              value={validFrom}
              onChange={(e) => setValidFrom(e.target.value)}
              className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-xs text-slate-100 font-mono outline-none focus:border-sky-500"
            />
          </div>

          <div className="space-y-1">
            <label className="text-xs text-slate-300 font-semibold block">Geldig tot</label>
            <input
              type="datetime-local"
              value={validTo}
